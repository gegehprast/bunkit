...
describe("@bunkit/server - Basic functionality", () => {
	...
	test("should handle multiple response schemas using .responses()", () => {
		...
		createRoute("POST", "/api/todos")
			...
			.handler(({ body, res }) => {
				if (!body.title.trim()) {
					return res.badRequest({ message: "Title cannot be empty", code: "INVALID_TITLE" })
				}
	...
	test("should register route-level middlewares", () => {
		...
		const firstMiddleware = async ({ next }: { next: () => Promise<Response | undefined> }) => {
			return next()
		}

		const secondMiddleware = async ({ next }: { next: () => Promise<Response | undefined> }) => {
			return next()
		}
	...
	test("should register middleware that can short-circuit", () => {
		...
		const authMiddleware = async ({ res }: { res: { unauthorized: (error: { message: string }) => Response } }) => {
	...
	test("should demonstrate handler return type validation", () => {
		...
		createRoute("POST", "/api/user")
			...
			.handler(({ body, res }) => {
				...
				if (exists) {
					return res.custom(
						JSON.stringify({ message: "User already exists", existingId: "123" }),
						{
							status: 409,
